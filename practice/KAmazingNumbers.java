...
public class KAmazingNumbers {
    ...
    private static void solve_kro(FastScanner sc) throws Exception{
    	...
    	for(int key = 1; key<=n; key++){
    		if(map[key]==null || map[key].isEmpty()) continue;
    		List<Integer> lst = map[key];
    		int max = lst.get(0)+1;
    		for(int i=1;i<lst.size();i++){
    			max = Math.max(max, lst.get(i)-lst.get(i-1));
    		}
    		max = Math.max(max, n-lst.get(lst.size()-1));
    		for(int i=max-1;i<n && ans[i]==-1; i++){
    			ans[i] = key;
    		}
    	}
