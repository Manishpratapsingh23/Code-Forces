...
public class GoodIndices {
    ...
    private static void solve_kro(FastScanner sc) throws Exception{
    	...
    	int pse[] = prevSmallerElement(n,arr);
    	int nse[] = nextSmallerElement(n,arr);
    	out = new StringBuilder();
    	for(int i=0;i<n;i++){
    		if(pse[i]==-1 || nse[i]==-1){
    			out.append(-1).append(" ");
    		} else {
    			out.append(nse[i] - pse[i] - 2).append(" ");
    		}
    	}
    ...
    static int[] prevSmallerElement(int n, int arr[]){
    	int ans[] = new int[n];
    ...
    static int[] nextSmallerElement(int n ,int arr[]){
    	int ans[] = new int[n];
    	Stack<Integer> stack = new Stack<>();
    	for(int i=n-1;i>=0;i--){
    		while(!stack.isEmpty() && arr[stack.peek()]>=arr[i]){
