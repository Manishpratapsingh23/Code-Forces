...
public class OlyaAndGameWithArrays {
    ...
    private static void solve_kro(Scanner sc){
        int n = sc.nextInt();
        //int m = sc.nextInt();
        long firstMinimum = Integer.MAX_VALUE;
        ...
        for(int i=0;i<n;i++){
        	...
        	Arrays.sort(a);
        	second.add(a[1]);
        	firstMinimum = Math.min(firstMinimum, a[0]);
        }
        Collections.sort(second);
        long ans = 0;
        for(long l : second) ans+=l;
        firstMinimum = Math.min(firstMinimum, second.get(0));
        ans = ans+firstMinimum-second.get(0);
